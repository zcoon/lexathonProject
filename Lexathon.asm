...
.text
main:
	#jal cleanDuplicateFile		#should be used when starting a new game, or perhaps when a game finishes
	
	#jal checkWordDictionary		#should be one of the many checks
	#returns a 1 in $v1 if word is in the dictionary
	
	#jal duplicateCheck		#should be one of the many checks
	#returns a 1 in $v1 if word is a duplicate
	
	#jal writeWordToDuplicateFile	#should be used when a word has passed all the checks, and is valid
...
duplicateWriteDo: 
   	lb $t2, userWord($t0)
	beq $t2, $t1, endDuplicateWriteDo     #calculates the length of myword and stores it in $t0	
    	addi $t0, $t0, 1
    	j duplicateWriteDo
endDuplicateWriteDo:		
	...
	li $v0, 13
   	...
   	syscall  # File descriptor gets returned in $v0
    	...
    	bne $t0, 4, duplicateCheck5
    	li $a2, 4  
    	j doneDuplicateCheck
duplicateCheck5: 
	bne $t0, 5, duplicateCheck6
    	li $a2, 5  
    	j doneDuplicateCheck
duplicateCheck6: 
	bne $t0, 6, duplicateCheck7	#These checks are necessary becuase writing to the file 
    	li $a2, 6  		#requires the hardcoded length of the word to be written
    	j doneDuplicateCheck		#so we need to hardcode all 6 options
duplicateCheck7: 
	bne $t0, 7, duplicateCheck8
    	li $a2, 7  
    	j doneDuplicateCheck
duplicateCheck8: 
	bne $t0, 8, duplicateCheck9
    	li $a2, 8  
    	j doneDuplicateCheck
duplicateCheck9: 
    	li $a2, 9  
